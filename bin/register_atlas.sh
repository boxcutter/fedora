...
get_short_description() {
    ...
    21)
        ...
        ;;
    20)
        PRETTY_VERSION="20 Heisenbug"
       ;;
    19)
        PRETTY_VERSION="19 Schrödinger's Cat"
        ;;
    18)
        PRETTY_VERSION="18 Spherical Cow"
        ;;
    esac
...
create_description() {
    ...
    DESCRIPTION="Fedora ${PRETTY_VERSION}${DESKTOP_STRING} (${BIT_STRING})${DOCKER_STRING}
...
atlas_publish() {
    ...
    BOXCUTTER_BASE_URL=http://cdn.boxcutter.io/fedora
    if [[ -e ${VMWARE_BOX_FILE} ]]; then
